...
<style>
    ...
    /* 修复点 1: 四色动态液态背景 (Top-Left to Bottom-Right) */
    ...
    .stApp, div.main {{
        background: linear-gradient(135deg, var(--theme-color), var(--theme-dark), var(--theme-light-gray), var(--theme-alpha)) !important;
    ...
    /* 修复点 2: L1 绝对平衡与对齐 (垂直居中) */[data-testid="stHorizontalBlock"] {{
...
# 修复点 4: 核心修复：影子状态代理 (彻底解决 StreamlitAPIException)
...
with st.sidebar:
    ...
    # 修复点 4: 侧边栏报表 (移至侧边栏，支持周/月/年及环比逻辑)
    with st.container():
    ...
    with st.expander("Modify", expanded=False):
        if config["subjects"]:
            ...
            if mod_type == "Parent":
                ...
                # 修复点 4: 严禁绑定 key，使用 value 接收影子状态，避免双向绑定冲突
                ...
                if c_list:
                    ...
                    # 修复点 4: 严禁绑定 key，使用 value 接收影子状态
...
with col_l3_left:
    ...
    # 修复点 3: 彻底解决代码外泄，全量遍历所有科目，使用纯 HTML 字符串拼接并一次性渲染
...
with col_l3_right:
    ...
    # 修复点 5: 动态标尺 (Gauge Max 随维度自动切换，精度保留一位小数)
