...
<style>
    ...
    /* 4. HTML Details Card (Gallery UX Evolution) */
    ...
    [data-testid="stExpander"] summary {{
        ...
        font-weight: 600;
        color: var(--text-main);
    }}[data-testid="stExpander"] summary:hover {{ color: var(--theme-color); }}[data-testid="stExpander"] div[role="region"] {{ background: transparent !important; padding: 0 28px 28px 28px !important; }}
    ...
    /* 9. Sidebar & Dialog */[data-testid="stSidebar"] {{
...
def sync_p_rn():
    """Callback: Sync Parent Rename Input instantly"""
    st.session_state.m_p_rn_input = st.session_state.m_p_sel

def sync_c_rn():
    """Callback: Sync Child Rename Input instantly"""
    st.session_state.m_c_rn_input = st.session_state.m_c_sel

def sync_p_c_rn():
    """Callback: Sync Child Rename Input when Parent changes"""
...
# ==========================================
# vertical_alignment="center" ensures pixel-perfect vertical centering
...
with col_l3_left:
    ...
    for idx, (parent, details) in enumerate(config["subjects"].items()):
        ...
        # 运行态解耦：仅在 idle 状态且被选中时高亮，计时中保持普通玻璃质感
        is_active = (sel_parent == parent) and (st.session_state.timer_state == 'idle')
        ...
        with gallery_cols[idx % 2]:
            # 卡片即组件：使用原生 HTML <details> 实现无缝展开
            st.markdown(f"""
