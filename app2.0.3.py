...
<style>
    ...
    /* 7. Sidebar Expander (Laboratory Card UI) */
...
with st.sidebar:
    ...
    # 3. Laboratory (Unified Glass Cards)
    ...
    with st.expander("Modify", expanded=False):
        if config["subjects"]:
            ...
            if mod_type == "Parent":
                ...
                mod_p = st.selectbox("Select", p_list, key="m_p_sel")
                
                # Robust Rename Logic: Bind value directly to selectbox output
                new_rn_name = st.text_input("Rename", value=mod_p, key="m_p_rn_input")
                ...
                p_list = list(config["subjects"].keys())
                mod_p_c = st.selectbox("Select Parent", p_list, key="m_c_p_sel")
                c_list = list(config["subjects"][mod_p_c]["children"].keys())
                
                if c_list:
                    mod_c = st.selectbox("Select Task", c_list, key="m_c_sel")
                    
                    # Robust Rename Logic for Child
                    new_c_name = st.text_input("Rename", value=mod_c, key="m_c_rn_input")
...
with col_l3_left:
    ...
    for idx, (parent, details) in enumerate(config["subjects"].items()):
        ...
        with gallery_cols[idx % 2]:
            # 卡片即组件：使用原生 HTML <details> 实现无缝展开，移除了多余的文本引导
            st.markdown(f"""
...
with col_l3_right:
    ...
    # 2. Monochromatic Pie Chart (Lead Lines Enabled)
    st.markdown("<div class='glass-card' style='padding: 24px;'>", unsafe_allow_html=True)
