...
bg_gradient = f"linear-gradient(135deg, {safe_theme_color}33 0%, #cfd9df 50%, #e2ebf0 100%)"
...
<style>
    ...
    [data-testid="stMarkdownContainer"], .element-container, .stMain,[data-testid="stMetric"], .stPlotlyChart, .stMarkdown {{
    ...
    /* 7. Main Gallery Expanders (Card as Switch) */
...
# Initialize controlled inputs for Modify
...
with st.sidebar:
    ...
    # 3. Laboratory (Glassmorphism Expanders)
    ...
    with st.expander("Modify", expanded=False):
        if config["subjects"]:
            ...
            if mod_type == "Parent":
                ...
                new_rn_target = st.number_input("Target", min_value=1.0, value=float(config["subjects"][mod_p].get("target_hours", 50.0)), step=1.0, key="m_p_tg", disabled=has_children)
                ...
                if c_list:
                    ...
                    new_c_tg = st.number_input("Target", min_value=1.0, value=float(config["subjects"][mod_p_c]["children"][mod_c].get("target_hours", 10.0)), step=1.0, key="m_c_tg")
...
# ==========================================
# vertical_alignment="center" natively aligns L1 components
...
with col_l1_left:
    ...
    if not parent_subjects:
        ...
        try:
            sel_index = parent_subjects.index(st.session_state.selected_subject) if st.session_state.selected_subject in parent_subjects else 0
        except ValueError:
            sel_index = 0
        ...
        with c_sel1:
            ...
            if st.session_state.selected_subject is None:
                st.session_state.selected_subject = sel_parent
...
with col_l3_left:
    ...
    for idx, (parent, details) in enumerate(config["subjects"].items()):
        ...
        with gallery_cols[idx % 2]:
            # 卡片即开关：Expander 整体作为卡片
            with st.expander(parent, expanded=is_selected):
                ...
                if not children:
                    ...
                    for child, c_details in children.items():
                        ...
                        <div style="margin-bottom: 12px;">
                            ...
                            <div class="pg-track" style="height: 4px; margin: 6px 0;"><div class="pg-fill" style="width: {c_prog}%; background-color: var(--theme-color);"></div></div>
                        </div>
                        """
...
with col_l3_right:
    ...
    # 1. Today's Progress Gauge (6h Max Precision)
    ...
    # 2. Monochromatic Pie Chart (Outside text)
    st.markdown("<div class='glass-card' style='padding: 24px;'>", unsafe_allow_html=True)
