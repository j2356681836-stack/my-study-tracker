...
# ==========================================
# 2. Deep Liquid Glass Perfection & CSS
# ==========================================
...
# Dynamic background with 15 hex alpha (~8% opacity)
bg_gradient = f"linear-gradient(135deg, {theme_color}15 0%, #f8f9fc 100%)"
...
<style>
    ...
    /* Eradicate White Blocks & Bars */
    .st-emotion-cache-1wivap2, .st-emotion-cache-1104q3y, .st-emotion-cache-16txtl3, 
    .st-emotion-cache-1y4p8pa, .st-emotion-cache-1n76uvr, .st-emotion-cache-18ni7ap,[data-testid="stVerticalBlock"], [data-testid="stHorizontalBlock"],[data-testid="stVerticalBlockBorderWrapper"], [data-testid="stHeader"] {{
    ...
    .kpi-value {{ color: var(--theme-color); font-family: 'Outfit', sans-serif; font-size: 3.2rem; font-weight: 700; line-height: 1; letter-spacing: -1px; }}
    ...
    /* Expander as Glass Card (Gallery & Sidebar) */
    ...
    .pg-track {{ width: 100%; height: 6px; background: rgba(255,255,255,0.6); border-radius: 3px; overflow: hidden; margin: 10px 0; box-shadow: inset 0 1px 2px rgba(0,0,0,0.05); }}
    ...
    div[role="radiogroup"] {{ justify-content: flex-end; gap: 12px; }}
...
with st.sidebar:
    ...
    with st.expander("Create", expanded=False):
        st.markdown("<div style='font-size: 0.85rem; color: var(--text-muted); margin-bottom: 8px; font-weight: 500;'>Parent Subject</div>", unsafe_allow_html=True)
        ...
        st.markdown("<div style='font-size: 0.85rem; color: var(--text-muted); margin-bottom: 8px; font-weight: 500;'>Child Task</div>", unsafe_allow_html=True)
        if config["subjects"]:
...
with col_l1_center:
    if parent_subjects:
        if st.session_state.timer_state == 'idle':
            st.markdown(f"""
                <div style="text-align: center; font-family: 'Outfit', sans-serif; font-size: 4.5rem; font-weight: 300; color: var(--theme-color); line-height: 1; margin-bottom: 16px; letter-spacing: -2px;">
                    00:00:00
                </div>
            """, unsafe_allow_html=True)
            ...
            <style>
                ...
                body {{ margin: 0; display: flex; justify-content: center; align-items: center; background: transparent; color: {theme_color}; font-family: 'Outfit', sans-serif; font-size: 4.5rem; font-weight: 300; line-height: 1; letter-spacing: -2px; }}
            </style>
...
with col_l3_right:
    ...
    # Fix Plotly ValueError by ensuring range max is strictly > 0
    max_gauge_val = max(t_hours, y_hours)
    gauge_range_max = max_gauge_val if max_gauge_val > 0 else 1.0

    fig_gauge = go.Figure(go.Indicator(
        mode = "gauge+number+delta",
        value = t_hours,
        title = {'text': "Today vs Yesterday (h)", 'font': {'size': 14, 'color': '#6E6E73', 'family': 'Inter'}},
        delta = {'reference': y_hours, 'increasing': {'color': theme_color}},
        gauge = {
            'axis': {'range': [None, gauge_range_max], 'tickwidth': 1, 'tickcolor': "rgba(255,255,255,0)"},
    ...
    st.markdown("<div class='glass-card' style='padding: 20px;'>", unsafe_allow_html=True)
    # Fix Plotly Pie Chart empty data issue
    if not filtered_df.empty and total_hours > 0:
        ...
        fig_pie.update_layout(
            title={'text': "Time Distribution", 'font': {'size': 14, 'color': '#6E6E73', 'family': 'Inter'}, 'x': 0.5, 'xanchor': 'center'},
...
fig_heat = go.Figure(data=go.Heatmap(
    ...
    x=heatmap_df['x'],
    y=heatmap_df['y'],
    colorscale=[[0, 'rgba(255,255,255,0.5)'],[1, theme_color]],
    xgap=4, ygap=4,
    showscale=False,
    hoverinfo='text',
...
fig_heat.update_layout(
    ...
    yaxis=dict(
        ...
        autorange='reversed',
        tickfont=dict(color="#6E6E73", family="Inter, sans-serif", size=12)
    )
)
