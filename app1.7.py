...
# ==========================================
# 2. Liquid Glass Perfection & CSS Injection
# ==========================================
...
# Dynamic background with 22 hex alpha (~13% opacity)
bg_gradient = f"linear-gradient(135deg, {theme_color}22 0%, #f0f2f6 100%)"
...
<style>
    ...
    .st-emotion-cache-1y4p8pa, .st-emotion-cache-1n76uvr, .st-emotion-cache-18ni7ap,[data-testid="stVerticalBlock"], [data-testid="stHorizontalBlock"] {{
    ...
    .kpi-value {{ color: var(--theme-color); font-family: 'Outfit', sans-serif; font-size: 2.5rem; font-weight: 600; line-height: 1; letter-spacing: -1px; }}
    .kpi-value span {{ font-size: 1.1rem; color: var(--text-muted); margin-left: 4px; font-weight: 500; letter-spacing: 0; }}
    ...
    .pg-track {{ width: 100%; height: 6px; background: rgba(255,255,255,0.5); border-radius: 3px; overflow: hidden; margin: 10px 0; box-shadow: inset 0 1px 2px rgba(0,0,0,0.05); }}
    ...
    if parent_subjects:
        if st.session_state.timer_state == 'idle':
            ...
            <style>
                ...
                body {{ margin: 0; display: flex; justify-content: center; background: transparent; color: {theme_color}; font-family: 'Outfit', sans-serif; font-size: 4.5rem; font-weight: 300; line-height: 1; letter-spacing: -2px; }}
            </style>
