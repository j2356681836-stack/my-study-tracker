...
<style>
    ...
    [data-testid="stMetric"], .stPlotlyChart, .stMarkdown {{
    ...
    .active-glass-card {{
        background: {safe_theme_color}E6 !important;
        backdrop-filter: blur(30px) !important;
    ...
    /* 7. Expander Logic (Sidebar vs Main) */
...
if 'm_p_rn' not in st.session_state: st.session_state.m_p_rn = ""
if 'm_c_rn' not in st.session_state: st.session_state.m_c_rn = ""
...
with st.sidebar:
    ...
    with st.expander("Modify", expanded=False):
        if config["subjects"]:
            ...
            if mod_type == "Parent":
                mod_p = st.selectbox("Select", list(config["subjects"].keys()), key="m_p_sel", on_change=sync_p_rn)
                if st.session_state.m_p_rn == "": st.session_state.m_p_rn = mod_p
                
                new_rn_name = st.text_input("Rename", key="m_p_rn")
                ...
                if children_list:
                    mod_c = st.selectbox("Select Task", children_list, key="m_c_sel", on_change=sync_c_rn)
                    if st.session_state.m_c_rn == "": st.session_state.m_c_rn = mod_c
                    
                    new_c_name = st.text_input("Rename", key="m_c_rn")
...
# ==========================================
# Using Streamlit 1.30+ vertical_alignment for pixel-perfect centering
...
with col_l3_left:
    ...
    for idx, (parent, details) in enumerate(config["subjects"].items()):
        ...
        with gallery_cols[idx % 2]:
            # Top half: HTML Card
            st.markdown(f"""
            <div class="{active_class}" style="padding: 24px 24px 16px 24px; border-radius: 28px 28px 0 0; border-bottom: none; margin-bottom: 0;">
