...
def init_system():
    """Initialize data files with defensive file creation."""
...
def generate_palette(base_hex, n=5):
    """Generate color palette, ensuring strictly 6-digit hex for Plotly compatibility."""
    base_hex = base_hex.lstrip('#')[:6] # Strip alpha if present
...
clean_theme_color = theme_color[:7] # Safe hex for Plotly (remove alpha)
palette = generate_palette(clean_theme_color, max(len(config["subjects"]), 5))

# Deep Liquid Gradient
bg_gradient = f"linear-gradient(135deg, {clean_theme_color}25 0%, #c3cfe2 50%, #9bc5c3 100%)"
...
<style>
    ...
    /* 1. Global Deep Liquid Glass Background */
    .stApp {{
    ...
    /* 2. THE WHITE BAR EXTERMINATOR: Force Transparency */
    ...
    [data-testid="stMarkdownContainer"], .element-container, .stMain, [data-testid="stMetric"] {{
    ...
    /* 4. KPI Typography (Bold & Distinct) */
    .kpi-container {{ display: flex; flex-direction: column; justify-content: center; height: 130px; text-align: center; }}
    .kpi-title {{ color: var(--text-muted); font-size: 0.85rem; font-weight: 600; text-transform: uppercase; letter-spacing: 1.5px; margin-bottom: 8px; }}
    .kpi-value {{ color: var(--theme-color); font-family: 'Inter', sans-serif; font-size: 3.5rem; font-weight: 700; line-height: 1; letter-spacing: -1px; text-shadow: 0 2px 10px rgba(255,255,255,0.3); }}
    ...
    [data-testid="stExpander"] div[role="region"] {{ background: transparent !important; padding: 0 24px 24px 24px !important; }}
    ...
    /* Force L1 columns to center content vertically */
...
# ==========================================
# 4. Sidebar: Management Laboratory
# ==========================================
with st.sidebar:
    st.markdown("<h2 style='font-family: Outfit; font-weight: 600; margin-bottom: 32px;'>Laboratory</h2>", unsafe_allow_html=True)
    ...
    with st.expander("Create", expanded=False):
        st.markdown("<div style='font-size: 0.85rem; color: var(--text-muted); font-weight: 600;'>Parent Subject</div>", unsafe_allow_html=True)
        new_parent = st.text_input("Name", key="c_p_name", label_visibility="collapsed")
        ...
        st.markdown("<div style='margin: 16px 0; border-top: 1px solid rgba(0,0,0,0.05);'></div>", unsafe_allow_html=True)
        
        st.markdown("<div style='font-size: 0.85rem; color: var(--text-muted); font-weight: 600;'>Child Task</div>", unsafe_allow_html=True)
        if config["subjects"]:
            sel_p_for_c = st.selectbox("Parent", list(config["subjects"].keys()), key="c_c_parent", label_visibility="collapsed")
            new_child = st.text_input("Task Name", key="c_c_name", label_visibility="collapsed")
...
# ==========================================
# 6. Central Core L1: Trinity Alignment
# ==========================================
# Using nested columns for the left side to get 1 Row 2 Cols
...
with col_l1_left:
    ...
    if not parent_subjects:
        st.markdown("<div style='color: var(--text-muted); font-weight: 500;'>Awaiting Configuration</div>", unsafe_allow_html=True)
        sel_parent, sel_child = None, None
    else:
        # Nested columns for 1 Row layout
...
with col_l1_center:
    if parent_subjects:
        if st.session_state.timer_state == 'idle':
            st.markdown(f"""
                <div style="text-align: center; font-family: 'Outfit', sans-serif; font-size: 4.8rem; font-weight: 300; color: {clean_theme_color}; line-height: 1; margin-bottom: 12px; letter-spacing: -2px;">
                    00:00:00
                </div>
            """, unsafe_allow_html=True)
            ...
            # JS Timer for smoothness
            timer_html = f"""
            <style>
                @import url('https://fonts.googleapis.com/css2?family=Outfit:wght@300&display=swap');
                body {{ margin: 0; display: flex; justify-content: center; align-items: center; background: transparent; color: {clean_theme_color}; font-family: 'Outfit', sans-serif; font-size: 4.8rem; font-weight: 300; line-height: 1; letter-spacing: -2px; }}
            </style>
...
# ==========================================
# 7.5 One-Click Report Dialog
# ==========================================
@st.dialog("Weekly Insight Report")
def show_report_dialog():
    # Calculate Weekly Stats
    start_week = now - timedelta(days=now.weekday())
    w_df = df[df['timestamp'].dt.date >= start_of_week.date()]
    w_hours = w_df['duration_minutes'].sum() / 60 if not w_df.empty else 0
    w_top = w_df.groupby('parent_subject')['duration_minutes'].sum().idxmax() if not w_df.empty else "None"
    w_score = w_df['focus_score'].mean() if not w_df.empty else 0.0

    st.markdown(f"""
    <div style="text-align: center; padding: 20px;">
        <h3 style="color: {clean_theme_color}; font-family: 'Outfit'; margin-bottom: 24px;">Weekly Summary</h3>
        <div style="display: flex; justify-content: space-around; margin-bottom: 24px;">
            <div><div style="font-size:0.8rem; color:#888;">HOURS</div><div style="font-size:1.5rem; font-weight:700;">{w_hours:.1f}</div></div>
            <div><div style="font-size:0.8rem; color:#888;">TOP FOCUS</div><div style="font-size:1.5rem; font-weight:700;">{w_top}</div></div>
            <div><div style="font-size:0.8rem; color:#888;">SCORE</div><div style="font-size:1.5rem; font-weight:700;">{w_score:.1f}</div></div>
        </div>
        <p style="color: #666; font-style: italic;">"Great things are done by a series of small things brought together."</p>
    </div>
    """, unsafe_allow_html=True)

# Button to trigger dialog (placed neatly below KPIs)
...
with col_l3_left:
    ...
    for idx, (parent, details) in enumerate(config["subjects"].items()):
        ...
        if target_h <= 0: target_h = 1 # Div/0 Safety
        ...
        with gallery_cols[idx % 2]:
            ...
            with st.expander(f"{parent}  |  {current_h:.1f}h / {target_h}h"):
                ...
                <div class="pg-label" style="margin-bottom: 16px;">
                    <span>Overall Progress</span><span>{progress_pct:.0f}%</span>
                </div>
                """, unsafe_allow_html=True)
                    ...
                    for child, c_details in children.items():
                        ...
                        <div style="margin-top: 12px;">
                            <div class="pg-label" style="font-size: 0.8rem;">
                                <span>{child}</span><span>{c_current_h:.1f}h / {c_target_h}h</span>
                            </div>
...
with col_l3_right:
    ...
    # Robust Plotly Fix: ValueError protection for gauge range
    ...
    fig_gauge = go.Figure(go.Indicator(
        ...
        delta = {'reference': y_hours, 'increasing': {'color': clean_theme_color}},
        gauge = {
            'axis': {'range': [0, gauge_max], 'tickwidth': 1, 'tickcolor': "rgba(255,255,255,0)"},
            'bar': {'color': clean_theme_color},
            'bgcolor': "rgba(255,255,255,0.3)",
            'borderwidth': 0,
            'threshold': {'line': {'color': f"{clean_theme_color}88", 'width': 2}, 'thickness': 0.75, 'value': y_hours}
        }
    ))
    ...
    st.markdown("<div class='glass-card' style='padding: 24px;'>", unsafe_allow_html=True)
    # Robust Plotly Fix: Empty pie chart check
    if not filtered_df.empty and total_hours > 0:
        ...
        fig_pie.add_annotation(text=f"<b>{total_hours:.1f}h</b>", x=0.5, y=0.5, font_size=24, showarrow=False, font_color=clean_theme_color)
...
# Hardcoded for 2026 as requested
curr_year = 2026
...
fig_heat = go.Figure(data=go.Heatmap(
    ...
    x=heatmap_df['x'],
    y=heatmap_df['y'],
    colorscale=[[0, 'rgba(255,255,255,0.4)'],[1, clean_theme_color]],
    xgap=4, ygap=4,
    showscale=False,
    hoverinfo='text',
