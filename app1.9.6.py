...
def sanitize_hex(color_str):
    """Force 6-digit HEX for Plotly stability to prevent ValueError."""
...
# ==========================================
# 2. Deep Liquid Glass CSS & Monochromatic Engine
# ==========================================
...
<style>
    ...
    /* 3. L1 Absolute Vertical Alignment */
    [data-testid="stHorizontalBlock"] {{
        align-items: center !important;
    }}
    
    /* 4. Universal Glass Card */
    .glass-card {{
    ...
    /* 5. Active Gallery Card (Highlight) */
    .active-card {{
    ...
    .active-card .pg-title, .active-card .pg-label {{ color: #FFFFFF !important; }}
    ...
    .kpi-value {{ color: var(--theme-color); font-family: 'Outfit', sans-serif; font-size: 3.6rem; font-weight: 700; line-height: 1; letter-spacing: -1px; text-shadow: 0 4px 20px rgba(255,255,255,0.4); }}
    ...
    .active-card [data-testid="stExpander"] summary {{ color: rgba(255,255,255,0.8); }}
    .active-card[data-testid="stExpander"] summary:hover {{ color: #FFFFFF; }}[data-testid="stExpander"] div[role="region"] {{ background: transparent !important; padding: 0 !important; }}

    /* 9. Typography & Radio Alignment */
    .section-title {{ font-family: 'Outfit', sans-serif; font-size: 1.5rem; font-weight: 600; color: var(--text-main); margin-bottom: 20px; letter-spacing: -0.5px; }}[data-testid="stRadio"] > div {{ justify-content: flex-end; width: 100%; }}
    ...
    /* Custom Progress */
    .pg-track {{ width: 100%; height: 6px; background: rgba(255,255,255,0.5); border-radius: 3px; overflow: hidden; margin: 10px 0; }}
    ...
    .pg-title {{ font-family: 'Inter', sans-serif; font-size: 1.15rem; font-weight: 600; margin-bottom: 8px; color: var(--text-main); }}
</style>
""", unsafe_allow_html=True)

# ==========================================
# 3. State Management & Sync Callbacks
# ==========================================
if 'timer_state' not in st.session_state: st.session_state.timer_state = 'idle'
if 'start_time' not in st.session_state: st.session_state.start_time = None

# Callbacks for auto-filling rename inputs
...
with st.sidebar:
    # 1. Theme Color
    st.markdown("<div style='font-size: 0.85rem; color: var(--text-muted); font-weight: 600; margin-bottom: 8px;'>Theme Color</div>", unsafe_allow_html=True)
    new_color = st.color_picker("Theme Color", raw_theme_color, label_visibility="collapsed")
    ...
    with st.expander("Modify", expanded=False):
        if config["subjects"]:
            ...
            if mod_type == "Parent":
                ...
                if "m_p_rn" not in st.session_state: st.session_state.m_p_rn = st.session_state.m_p_sel
                
                mod_p = st.selectbox("Select", list(config["subjects"].keys()), key="m_p_sel", on_change=sync_parent_rename)
                new_rn_name = st.text_input("New Name", key="m_p_rn")
                ...
                if children_list:
                    ...
                    if "m_c_rn" not in st.session_state: st.session_state.m_c_rn = st.session_state.m_c_sel
                    
                    mod_c = st.selectbox("Select Task", children_list, key="m_c_sel", on_change=sync_child_rename)
                    new_c_name = st.text_input("New Name", key="m_c_rn")
...
# ==========================================
# 5. Central Core L1: Absolute Alignment
# ==========================================
...
if time_filter == "Today":
    ...
    compare_df = df[(df['timestamp'].dt.date >= compare_start.date()) & (df['timestamp'].dt.date < start_of_week.date())]
...
with col_l3_left:
    ...
    for idx, (parent, details) in enumerate(config["subjects"].items()):
        ...
        is_active = (sel_parent == parent)
        card_class = "glass-card active-card" if is_active else "glass-card"
        
        with gallery_cols[idx % 2]:
            st.markdown(f"""
            <div class="{card_class}" style="margin-bottom: 0; padding-bottom: 10px;">
                    ...
                    for child, c_details in children.items():
                        ...
                        <div style="margin-top: 8px;">
                            <div class="pg-label" style="font-size: 0.8rem;">
                                <span>{child}</span><span>{c_current_h:.1f}h / {c_target_h:.1f}h</span>
                            </div>
                            <div class="pg-track" style="height: 4px; margin: 4px 0;">
...
with col_l3_right:
    ...
    # 1. Today's Progress Dashboard (Gauge)
    st.markdown("<div class='glass-card' style='padding: 24px;'>", unsafe_allow_html=True)
    
    today_df = df[df['timestamp'].dt.date == now.date()]
    today_hours = today_df['duration_minutes'].sum() / 60 if not today_df.empty else 0
    
    yesterday_df = df[df['timestamp'].dt.date == (now.date() - timedelta(days=1))]
    yesterday_hours = yesterday_df['duration_minutes'].sum() / 60 if not yesterday_df.empty else 0
    
    # Calculate Daily Target (Total Target / 365)
    total_target_hours = sum(get_parent_target(p) for p in config["subjects"])
    daily_target = max(total_target_hours / 365, 0.1) # Prevent 0
    
    gauge_max = max(daily_target, today_hours, yesterday_hours) * 1.2

    fig_gauge = go.Figure(go.Indicator(
        mode = "gauge+number+delta",
        value = today_hours,
        number = {'valueformat': ".1f"},
        title = {'text': "Today's Progress Dashboard", 'font': {'size': 14, 'color': '#5A5A5E', 'family': 'Inter'}},
        delta = {'reference': yesterday_hours, 'increasing': {'color': safe_theme_color}, 'valueformat': ".1f"},
        gauge = {
            'axis': {'range':[0, gauge_max], 'tickwidth': 1, 'tickcolor': "rgba(255,255,255,0)"},
            'bar': {'color': safe_theme_color},
            'bgcolor': "rgba(255,255,255,0.3)",
            'borderwidth': 0,
            'threshold': {'line': {'color': "#FFFFFF", 'width': 3}, 'thickness': 0.75, 'value': daily_target}
        }
    ))
...
# ==========================================
# 8. Central Core L4: 2026 Heatmap
# ==========================================
