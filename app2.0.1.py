...
def get_parent_target(parent_name):
    ...
    if not children:
        return max(0.1, float(parent_data.get("target_hours", 1.0)))
    return max(0.1, sum(float(child_data.get("target_hours", 1.0)) for child_data in children.values()))
...
<style>
    ...
    /* 1. Global Dynamic Gradient Background */
    ...
    .stApp {{
        background: linear-gradient(-45deg, {safe_theme_color}22, #f5f7fa, #c3cfe2, {safe_theme_color}33) !important;
    ...
    [data-testid="stVerticalBlock"],[data-testid="stHorizontalBlock"],[data-testid="stVerticalBlockBorderWrapper"], [data-testid="stHeader"],[data-testid="stMarkdownContainer"], .element-container, .stMain,[data-testid="stMetric"], .stPlotlyChart, .stMarkdown {{
        background: transparent !important;
        background-color: transparent !important;
        border: none !important;
    }}
    .js-plotly-plot .plotly, .js-plotly-plot .plotly div {{ background-color: transparent !important; }}
...
# ==========================================
# 3. State Management & Callbacks (State Guard)
# ==========================================
if 'timer_state' not in st.session_state: st.session_state.timer_state = 'idle'
if 'start_time' not in st.session_state: st.session_state.start_time = None
if 'selected_subject' not in st.session_state: st.session_state.selected_subject = None

if 'm_p_rn_input' not in st.session_state: st.session_state.m_p_rn_input = ""
if 'm_c_rn_input' not in st.session_state: st.session_state.m_c_rn_input = ""

def sync_p_rn():
    st.session_state.m_p_rn_input = st.session_state.m_p_sel

def sync_c_rn():
    st.session_state.m_c_rn_input = st.session_state.m_c_sel

def sync_p_c_rn():
    parent = st.session_state.m_c_p_sel
    children = list(config["subjects"][parent]["children"].keys())
    st.session_state.m_c_rn_input = children[0] if children else ""

def sync_l1_parent():
    st.session_state.selected_subject = st.session_state.l1_p_sel

# --- Callbacks for Modify/Delete to prevent StreamlitAPIException ---
def save_parent_mod():
    mod_p = st.session_state.m_p_sel
    new_rn_name = st.session_state.m_p_rn_input
    new_rn_target = st.session_state.m_p_tg
    has_children = len(config["subjects"][mod_p]["children"]) > 0
    if new_rn_name != mod_p and new_rn_name:
    ...
    save_config(config)
    st.session_state.m_p_rn_input = ""

def save_child_mod():
    mod_p_c = st.session_state.m_c_p_sel
    mod_c = st.session_state.m_c_sel
    new_c_name = st.session_state.m_c_rn_input
    new_c_tg = st.session_state.m_c_tg
    if new_c_name != mod_c and new_c_name:
    ...
    save_config(config)
    st.session_state.m_c_rn_input = ""

def delete_parent_cb():
    del_p = st.session_state.d_p_sel
    del config["subjects"][del_p]
    save_config(config)

def delete_child_cb():
    del_p_c = st.session_state.d_c_p_sel
    del_c = st.session_state.d_c_sel
...
def show_report_dialog(period_type):
    ...
    growth = ((c_hours - p_hours) / p_hours) * 100 if p_hours > 0 else (100.0 if c_hours > 0 else 0.0)
...
with st.sidebar:
    ...
    # 移除了 expanded=False，让 Streamlit 原生记忆折叠状态
    ...
    with st.expander("Modify"):
        if config["subjects"]:
            ...
            if mod_type == "Parent":
                ...
                mod_p = st.selectbox("Select", p_list, key="m_p_sel", on_change=sync_p_rn)
                
                if not st.session_state.m_p_rn_input and p_list:
                    st.session_state.m_p_rn_input = mod_p
                
                st.text_input("Rename", key="m_p_rn_input")
                has_children = len(config["subjects"][mod_p]["children"]) > 0
                st.number_input("Target", min_value=1.0, value=float(config["subjects"][mod_p].get("target_hours", 50.0)), step=1.0, key="m_p_tg", disabled=has_children)
                
                st.button("Save", key="m_p_btn", type="primary", use_container_width=True, on_click=save_parent_mod)
            else:
                p_list = list(config["subjects"].keys())
                mod_p_c = st.selectbox("Select Parent", p_list, key="m_c_p_sel", on_change=sync_p_c_rn)
                c_list = list(config["subjects"][mod_p_c]["children"].keys())
                
                if c_list:
                    mod_c = st.selectbox("Select Task", c_list, key="m_c_sel", on_change=sync_c_rn)
                    if not st.session_state.m_c_rn_input:
                        st.session_state.m_c_rn_input = mod_c
                        
                    st.text_input("Rename", key="m_c_rn_input")
                    st.number_input("Target", min_value=1.0, value=float(config["subjects"][mod_p_c]["children"][mod_c].get("target_hours", 10.0)), step=1.0, key="m_c_tg")
                    
                    st.button("Save", key="m_c_btn", type="primary", use_container_width=True, on_click=save_child_mod)

    with st.expander("Delete"):
        if config["subjects"]:
            del_type = st.radio("Type", ["Parent", "Child"], horizontal=True, label_visibility="collapsed", key="del_rad")
            if del_type == "Parent":
                st.selectbox("Select", list(config["subjects"].keys()), key="d_p_sel")
                st.button("Confirm Delete", key="d_p_btn", type="primary", use_container_width=True, on_click=delete_parent_cb)
            else:
                ...
                if children_list:
                    st.selectbox("Task", children_list, key="d_c_sel")
                    st.button("Confirm Delete", key="d_c_btn", type="primary", use_container_width=True, on_click=delete_child_cb)
...
with col_l1_left:
    ...
    if not parent_subjects:
        ...
        # 确保 selected_subject 始终有效
        if st.session_state.selected_subject not in parent_subjects:
            st.session_state.selected_subject = parent_subjects[0] if parent_subjects else None
            
        sel_index = parent_subjects.index(st.session_state.selected_subject) if st.session_state.selected_subject else 0

        c_sel1, c_sel2 = st.columns(2)
        with c_sel1:
            sel_parent = st.selectbox("Subject", parent_subjects, index=sel_index, key="l1_p_sel", on_change=sync_l1_parent, disabled=(st.session_state.timer_state != 'idle'), label_visibility="collapsed")
...
with col_l3_left:
    ...
    for idx, (parent, details) in enumerate(config["subjects"].items()):
        ...
        is_selected = (st.session_state.selected_subject == parent)
        is_running = (st.session_state.timer_state == 'running')
        marker_class = f"marker-{parent.replace(' ', '-')}"
        
        # 计时态隔离：仅在 idle 且被选中时高亮
        if is_selected and not is_running:
            st.markdown(f"""
            <style>
            div[data-testid="stExpander"]:has(.{marker_class}) {{
                background: {safe_theme_color} !important;
                border: 1px solid rgba(255,255,255,0.5) !important;
                box-shadow: 0 12px 40px {safe_theme_color}66 !important;
            }}
            div[data-testid="stExpander"]:has(.{marker_class}) summary,
            div[data-testid="stExpander"]:has(.{marker_class}) summary p,
            div[data-testid="stExpander"]:has(.{marker_class}) summary svg {{
                color: #FFFFFF !important;
                fill: #FFFFFF !important;
            }}
            div[data-testid="stExpander"]:has(.{marker_class}) .pg-label,
            div[data-testid="stExpander"]:has(.{marker_class}) span {{
                color: rgba(255,255,255,0.9) !important;
            }}
            div[data-testid="stExpander"]:has(.{marker_class}) .pg-track {{
                background: rgba(0,0,0,0.2) !important;
            }}
            div[data-testid="stExpander"]:has(.{marker_class}) .pg-fill {{
        ...
        with gallery_cols[idx % 2]:
            with st.expander(parent):
                ...
                # Parent Progress
                st.markdown(f"""
                <div class="pg-track" style="margin-top: 0;"><div class="pg-fill" style="width: {progress_pct}%;"></div></div>
                <div class="pg-label" style="margin-top: 8px; margin-bottom: 20px;">
                    <span>{current_h:.1f}h / {target_h:.1f}h</span>
                    <span>{progress_pct:.1f}%</span>
                </div>
                """, unsafe_allow_html=True)
                
                # Children Tasks (单行压缩，防止 Markdown 误解析导致 HTML 泄露)
                children = details.get("children", {})
                if not children:
                    st.markdown("<div style='color: var(--text-muted); font-size: 0.85rem;'>No specific tasks configured.</div>", unsafe_allow_html=True)
                else:
                    tasks_html = ""
                    for child, c_details in children.items():
                        c_target = max(0.1, float(c_details.get("target_hours", 1.0)))
                        c_current_m = filtered_df[(filtered_df['parent_subject'] == parent) & (filtered_df['child_subject'] == child)]['duration_minutes'].sum() if not filtered_df.empty else 0
                        c_current_h = c_current_m / 60
                        c_prog = min((c_current_h / c_target) * 100, 100)
                        tasks_html += f"<div style='margin-bottom: 12px;'><div style='display: flex; justify-content: space-between; font-size: 0.85rem; font-weight: 500;'><span>{child}</span><span>{c_current_h:.1f}h / {c_target:.1f}h</span></div><div class='pg-track' style='height: 4px; margin: 6px 0;'><div class='pg-fill' style='width: {c_prog}%; background-color: var(--theme-color);'></div></div></div>"
                    st.markdown(f"<div style='border-top: 1px solid rgba(255,255,255,0.2); padding-top: 16px;'>{tasks_html}</div>", unsafe_allow_html=True)

with col_l3_right:
    st.markdown("<div class='section-title'>Insights</div>", unsafe_allow_html=True)
    
    # 1. Today's Progress Gauge (硬编码 6.0h 最大值)
    ...
    if time_filter == "Today": 
        target_period = total_target_hours / 365
        gauge_max = 6.0
    elif time_filter == "Week": 
        target_period = total_target_hours / 52
        gauge_max = max(target_period, total_hours) * 1.2
    elif time_filter == "Month": 
        target_period = total_target_hours / 12
        gauge_max = max(target_period, total_hours) * 1.2
    else: 
        target_period = total_target_hours
        gauge_max = max(target_period, total_hours) * 1.2
        
    target_period = max(0.1, target_period)
    ...
    fig_gauge = go.Figure(go.Indicator(
        ...
        gauge = {
            ...
            'borderwidth': 0,
            'threshold': {'line': {'color': 'white', 'width': 2}, 'thickness': 0.75, 'value': target_period}
        }
    ))
    ...
    # 2. Monochromatic Pie Chart (外部标签 + 引导线)
    st.markdown("<div class='glass-card' style='padding: 24px;'>", unsafe_allow_html=True)
    if not filtered_df.empty and total_hours > 0:
        ...
        fig_pie.update_traces(textposition='outside', textinfo='label+percent', marker=dict(line=dict(color='rgba(255,255,255,0.6)', width=1)))
        fig_pie.update_layout(
            title={'text': "Distribution", 'font': {'size': 14, 'color': '#5A5A5E', 'family': 'Inter'}, 'x': 0.5, 'xanchor': 'center'},
            showlegend=False,
            height=240, margin=dict(l=20, r=20, t=40, b=20), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font={'family': "Inter, sans-serif"}
        )
