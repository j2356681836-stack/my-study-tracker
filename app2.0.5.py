...
theme_dark = adjust_color(safe_theme_color, 0.7)
theme_light = adjust_color(safe_theme_color, 1.3)
theme_alpha = adjust_color(safe_theme_color, 1.0, 0.3)
...
<style>
    ...
    .stApp, div.main {{
        background: linear-gradient(-45deg, var(--theme-alpha), var(--theme-dark), var(--theme-light), rgba(255,255,255,0.85)) !important;
    ...
    /* 修复点 4: L1 绝对平衡与对齐 (垂直居中) */[data-testid="stHorizontalBlock"] {{
    ...
    /* Sidebar Expander (Laboratory Card UI) */
...
# 修复点 1: 核心修复：影子状态代理 (彻底解决 StreamlitAPIException)
...
with st.sidebar:
    ...
    # 修复点 5: 侧边栏报表 (移至侧边栏，支持周/月/年及环比逻辑)
    with st.container():
    ...
    with st.expander("Modify", expanded=False):
        if config["subjects"]:
            ...
            if mod_type == "Parent":
                ...
                # 修复点 1: 严禁绑定 key，使用 value 接收影子状态
...
with col_l3_left:
    ...
    # 修复点 3: 彻底解决代码外泄，使用纯 HTML 字符串拼接并一次性渲染
