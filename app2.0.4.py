...
def get_darker_color(hex_color, factor=0.6):
...
dark_theme_color = get_darker_color(safe_theme_color)
...
<style>
    ...
    /* 1. 4-Color Dynamic Liquid Background */
    @keyframes liquid-bg {{
    ...
    .stApp {{
        background: linear-gradient(-45deg, {safe_theme_color}55, {dark_theme_color}77, #E2EBF0, rgba(255,255,255,0.9)) !important;
        background-size: 400% 400% !important;
        animation: liquid-bg 18s ease infinite !important;
        background-attachment: fixed !important;
        font-family: 'Inter', sans-serif;
        color: var(--text-main);
    }}

    /* 2. THE WHITE BAR KILLER & Pixel Perfect Alignment */
    ...
    /* 4. Grid Gallery & Immersive Cards */
    .gallery-grid {{
        display: grid;
    ...
    /* Hover Effects for Physical Feedback */
    ...
    /* 7. Sidebar Expander (Laboratory Card UI) */[data-testid="stSidebar"] [data-testid="stExpander"] {{
    ...
    [data-testid="stRadio"] {{ display: flex; justify-content: flex-end; width: 100%; }}
...
# ==========================================
# 3. State Management & Proxy Pattern
# ==========================================
if 'timer_state' not in st.session_state: st.session_state.timer_state = 'idle'
if 'start_time' not in st.session_state: st.session_state.start_time = None

# Shadow states for robust renaming (Fixes StreamlitAPIException)
    ...
    with st.expander("Modify", expanded=False):
        if config["subjects"]:
            ...
            if mod_type == "Parent":
                ...
                mod_p = st.selectbox("Select", p_list, key="m_p_sel", on_change=sync_p_name)
                ...
                p_list = list(config["subjects"].keys())
                mod_p_c = st.selectbox("Select Parent", p_list, key="m_c_p_sel", on_change=sync_p_c_name)
                c_list = list(config["subjects"][mod_p_c]["children"].keys())
                
                if c_list:
                    mod_c = st.selectbox("Select Task", c_list, key="m_c_sel", on_change=sync_c_name)
...
with col_l1_left:
    ...
    if not parent_subjects:
        ...
        with c_sel1:
            # index=None prevents auto-highlighting the first card on load
...
with col_l3_left:
    ...
    # Pure HTML Grid Construction (Prevents Markdown Source Leak & Ensures Alignment)
    ...
    for parent, details in config["subjects"].items():
        ...
        # Strict state check: Highlight ONLY if manually selected AND timer is idle.
...
with col_l3_right:
    ...
    if not filtered_df.empty and total_hours > 0:
        ...
        # Force percentages outside to enable lead lines
