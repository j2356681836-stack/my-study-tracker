...
def get_parent_target(parent_name):
    ...
    if not children:
        return max(0.1, parent_data.get("target_hours", 1))
    return max(0.1, sum(child_data.get("target_hours", 1) for child_data in children.values()))
...
# ==========================================
# 2. State Management & Callbacks (Req 1 Fix)
# ==========================================
...
def sync_p_rn(): st.session_state.m_p_rn_input = st.session_state.m_p_sel
def sync_c_rn(): st.session_state.m_c_rn_input = st.session_state.m_c_sel
def sync_p_c_rn():
...
# Callback: 彻底修复 Modify 时的 StreamlitAPIException
def on_save_parent():
    ...
    save_config(config)
    st.session_state.m_p_rn_input = "" # 在 Callback 中清空是安全的
...
# ==========================================
# 3. Deep Liquid Glass CSS Engine & Color Harmony
# ==========================================
...
<style>
    ...
    .stApp {{
        background: linear-gradient(-45deg, {safe_theme_color}44, #cfd9df, #e2ebf0, {safe_theme_color}66) !important;
    ...
    /* Req 3: 修复侧边栏收起后的展开箭头不可见问题 */[data-testid="collapsedControl"] {{
        visibility: visible !important;
        color: var(--text-main) !important;
    }}

    /* THE WHITE BAR KILLER (移除了 stVerticalBlockBorderWrapper 以支持原生卡片化) */
    ...
    [data-testid="stVerticalBlock"], [data-testid="stHorizontalBlock"], [data-testid="stHeader"],[data-testid="stMarkdownContainer"], .element-container, .stMain, [data-testid="stMetric"], .stMarkdown {{
        background: transparent !important;
        background-color: transparent !important;
        border: none !important;
    }}
    
    /* Req 5: 全局原生容器卡片化 (将 st.container(border=True) 转化为玻璃卡片) */[data-testid="stVerticalBlockBorderWrapper"] {{
    ...
    /* Universal Glass Card (For Custom HTML) */
    .glass-card {{
        background: var(--glass-bg) !important;
        backdrop-filter: blur(30px) !important;
        -webkit-backdrop-filter: blur(30px) !important;
    ...
    details.glass-card-detail > summary {{ list-style: none; outline: none; cursor: pointer; }}
    details.glass-card-detail > summary::-webkit-details-marker {{ display: none; }}
    ...
    details.active-glass-card-detail > summary {{ list-style: none; outline: none; cursor: pointer; }}
    details.active-glass-card-detail > summary::-webkit-details-marker {{ display: none; }}
    ...
    .stButton > button[data-baseweb="button"]:has(div p) {{ /* Target default */ }}
    button[kind="primary"] {{
        background: var(--theme-color) !important;
        color: #FFFFFF !important;
    ...
    /* Typography */
    .section-title {{ font-family: 'Outfit', sans-serif; font-size: 1.4rem; font-weight: 600; color: var(--text-main); margin-bottom: 16px; letter-spacing: -0.5px; }}
</style>
...
with st.sidebar:
    ...
    with st.container(border=True):
        st.markdown("<div style='font-size: 0.9rem; color: var(--text-main); font-weight: 600; margin-bottom: 8px;'>Report Generator</div>", unsafe_allow_html=True)
    ...
    # Req 3: 侧边栏菜单卡片化，并强调确认按钮
    with st.container(border=True):
        st.markdown("<div style='font-weight: 600; margin-bottom: 12px;'>Create</div>", unsafe_allow_html=True)
        ...
        if st.button("Add Subject", use_container_width=True, type="primary") and new_parent:
        ...
        if config["subjects"]:
            ...
            if st.button("Add Task", use_container_width=True, type="primary") and new_child:
    ...
    with st.container(border=True):
        st.markdown("<div style='font-weight: 600; margin-bottom: 12px;'>Modify</div>", unsafe_allow_html=True)
        if config["subjects"]:
            ...
            if mod_type == "Parent":
                ...
                if not st.session_state.m_p_rn_input and p_list:
                    st.session_state.m_p_rn_input = p_list[0]
                
                mod_p = st.selectbox("Select", p_list, key="m_p_sel", on_change=sync_p_rn)
                new_rn_name = st.text_input("Rename", key="m_p_rn_input")
                ...
                # 使用 Callback 修复报错
                st.button("Save", key="m_p_btn", use_container_width=True, type="primary", on_click=on_save_parent)
            else:
                p_list = list(config["subjects"].keys())
                ...
                if c_list:
                    if not st.session_state.m_c_rn_input:
                        st.session_state.m_c_rn_input = c_list[0]
                        
                    mod_c = st.selectbox("Select Task", c_list, key="m_c_sel", on_change=sync_c_rn)
                    new_c_name = st.text_input("Rename", key="m_c_rn_input")
                    new_c_tg = st.number_input("Target", min_value=1, value=config["subjects"][mod_p_c]["children"][mod_c].get("target_hours", 10), key="m_c_tg")
                    
                    # 使用 Callback 修复报错
                    st.button("Save", key="m_c_btn", use_container_width=True, type="primary", on_click=on_save_child)

    with st.container(border=True):
        st.markdown("<div style='font-weight: 600; margin-bottom: 12px;'>Delete</div>", unsafe_allow_html=True)
        if config["subjects"]:
            ...
            if del_type == "Parent":
                ...
                if st.button("Confirm Delete", key="d_p_btn", use_container_width=True, type="primary"):
                ...
                if children_list:
                    ...
                    if st.button("Confirm Delete", key="d_c_btn", use_container_width=True, type="primary"):
...
with col_l1_left:
    ...
    if not parent_subjects:
        ...
        with c_sel1:
            # Req 2: index=None 废除默认高亮，只有用户主动选择后才激活
        ...
        with c_sel2:
            if sel_parent is None:
                sel_child = None
                st.selectbox("Task", ["Select Subject First"], disabled=True, label_visibility="collapsed")
            else:
...
# ==========================================
# 7. Central Core L3: Interactive Gallery & Insights
# ==========================================
    ...
    for idx, (parent, details) in enumerate(config["subjects"].items()):
        ...
        if not children:
            ...
            for child, c_details in children.items():
                c_target = max(0.1, c_details.get("target_hours", 1))
                ...
                # Req 2: 压缩 HTML 字符串防止 Streamlit Markdown 解析器注入 <p> 标签导致源码泄露
        ...
        with gallery_cols[idx % 2]:
            # 强制使用 unsafe_allow_html=True 并压缩外层结构
            st.markdown(f"""<details class="{card_class}"><summary><div style="font-family:'Inter'; font-weight:600; font-size:1.1rem; margin-bottom:12px;">{parent}</div><div class="pg-track"><div class="pg-fill" style="width: {progress_pct}%;"></div></div><div class="pg-label" style="margin-top: 8px;"><span>{current_h:.1f}h / {target_h:.1f}h</span><span>{progress_pct:.1f}%</span></div></summary><div class="task-list-container" style="margin-top: 16px; padding-top: 16px; border-top: 1px solid rgba(255,255,255,0.2);">{tasks_html}</div></details>""", unsafe_allow_html=True)

with col_l3_right:
    st.markdown("<div class='section-title'>Insights</div>", unsafe_allow_html=True)
    
    # Req 5: 清理冗余 HTML 占位符，使用原生 container 配合 CSS 实现图表卡片化
    with st.container(border=True):
        ...
        fig_gauge = go.Figure(go.Indicator(
            ...
            gauge = {
                'axis': {'range':[0, 6], 'tickwidth': 1, 'tickcolor': "rgba(255,255,255,0)"},
    ...
    with st.container(border=True):
        if not filtered_df.empty and total_hours > 0:
            ...
            fig_pie.update_layout(
                ...
                showlegend=False, # 标签已在外部，隐藏底部图例以节省空间
...
# ==========================================
# 8. Central Core L4: Dynamic Heatmap
# ==========================================
# Req 7: 年份动态化
curr_year = datetime.now().year
st.markdown(f"<div class='section-title' style='margin-top: 16px;'>Annual Activity ({curr_year})</div>", unsafe_allow_html=True)
