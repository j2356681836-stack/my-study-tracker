...
def darken_color(hex_color, factor=0.6):
    """Generate a darker shade of the theme color for KPI text readability."""
    hex_color = hex_color.lstrip('#')
    if len(hex_color) != 6: hex_color = "007AFF"
    r, g, b = tuple(int(hex_color[i:i+2], 16) for i in (0, 2, 4))
    r, g, b = int(r * factor), int(g * factor), int(b * factor)
    return f"#{r:02x}{g:02x}{b:02x}"

# ==========================================
# 2. Deep Liquid Glass Visual Standards & CSS
# ==========================================
theme_color = config.get("theme_color", "#007AFF")
theme_dark = darken_color(theme_color)
...
<style>
    ...
    /* Deep Liquid Glass Global Background */
    .stApp {{
    ...
    .kpi-value {{ color: var(--theme-dark); font-family: 'Inter', sans-serif; font-size: 3rem; font-weight: 700; line-height: 1; letter-spacing: -1px; }}
    .kpi-value span {{ font-size: 1.2rem; color: var(--text-muted); margin-left: 4px; font-weight: 500; letter-spacing: 0; }}
    ...
    [data-testid="stExpander"] details {{ border: none !important; }}[data-testid="stExpander"] summary {{
    ...
    [data-testid="stExpander"] summary:hover {{ color: var(--theme-dark); }}

    /* Custom Progress Bars */
    .pg-track {{ width: 100%; height: 8px; background: rgba(255,255,255,0.6); border-radius: 4px; overflow: hidden; margin: 12px 0; box-shadow: inset 0 1px 3px rgba(0,0,0,0.05); }}
    .pg-fill {{ height: 100%; border-radius: 4px; transition: width 1s ease; background-color: var(--theme-color); }}
    .pg-label {{ display: flex; justify-content: space-between; font-size: 0.85rem; color: var(--text-muted); font-weight: 600; }}

    /* Typography */
    .section-title {{ font-family: 'Inter', sans-serif; font-size: 1.3rem; font-weight: 600; color: var(--text-main); margin-bottom: 24px; letter-spacing: -0.5px; }}
...
# ==========================================
with st.sidebar:
    st.markdown("<h2 style='font-family: Inter; font-weight: 600; margin-bottom: 32px; letter-spacing: -0.5px;'>Laboratory</h2>", unsafe_allow_html=True)
    ...
    with st.expander("Create", expanded=False):
        ...
        st.markdown("<hr style='margin: 16px 0; border: none; border-top: 1px solid rgba(255,255,255,0.5);'>", unsafe_allow_html=True)
...
with col_l1_center:
    if parent_subjects:
        if st.session_state.timer_state == 'idle':
            st.markdown(f"""
                <div style="text-align: center; font-family: 'Inter', sans-serif; font-size: 4.5rem; font-weight: 700; color: var(--theme-dark); line-height: 1; margin-bottom: 16px; letter-spacing: -2px;">
                    00:00:00
                </div>
            """, unsafe_allow_html=True)
            ...
            <style>
                ...
                body {{ margin: 0; display: flex; justify-content: center; background: transparent; color: {theme_dark}; font-family: 'Inter', sans-serif; font-size: 4.5rem; font-weight: 700; line-height: 1; letter-spacing: -2px; }}
            </style>
...
with col_l3_left:
    ...
    # 2x2 or 3x3 Grid Layout using columns
    ...
    for idx, (parent, details) in enumerate(config["subjects"].items()):
        ...
        with gallery_cols[idx % 2]:
            ...
            with st.expander(f"{parent}  |  {current_h:.1f}h / {target_h}h"):
                ...
                <div class="pg-label" style="margin-bottom: 20px;">
...
with col_l3_right:
    ...
    fig_gauge = go.Figure(go.Indicator(
        ...
        value = t_hours,
        title = {'text': "Today vs Yesterday (h)", 'font': {'size': 14, 'color': '#58585C', 'family': 'Inter'}},
        ...
        gauge = {
            ...
            'borderwidth': 0,
            'threshold': {'line': {'color': theme_dark, 'width': 2}, 'thickness': 0.75, 'value': y_hours}
        }
    ))
    ...
    if not filtered_df.empty:
        fig_pie = px.pie(filtered_df, names='parent_subject', values='duration_minutes', hole=0.75)
        fig_pie.update_traces(textposition='inside', textinfo='percent', marker=dict(line=dict(color='rgba(255,255,255,0.8)', width=2)))
        fig_pie.update_layout(
            title={'text': "Time Distribution", 'font': {'size': 14, 'color': '#58585C', 'family': 'Inter'}, 'x': 0.5, 'xanchor': 'center'},
        ...
        fig_pie.add_annotation(text=f"<b>{total_hours:.1f}h</b>", x=0.5, y=0.5, font_size=24, showarrow=False, font_color=theme_dark)
...
fig_heat.update_layout(
    ...
    yaxis=dict(
        ...
        autorange='reversed',
        tickfont=dict(color="#58585C", family="Inter, sans-serif", size=12)
    )
)
