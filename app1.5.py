...
# ==========================================
# 2. Liquid Glass Visual Standards & CSS
# ==========================================
...
<style>
    ...
    /* Soft Mesh Gradient Background to reveal Glassmorphism */
    .stApp {{
        background: radial-gradient(at 0% 0%, rgba(240,244,255,1) 0px, transparent 50%),
                    radial-gradient(at 100% 0%, rgba(255,240,245,1) 0px, transparent 50%),
                    radial-gradient(at 100% 100%, rgba(240,255,250,1) 0px, transparent 50%),
                    radial-gradient(at 0% 100%, rgba(245,240,255,1) 0px, transparent 50%);
    ...
    /* KPI Specifics */
    .kpi-container {{ display: flex; flex-direction: column; justify-content: center; height: 120px; text-align: center; }}
    ...
    .kpi-value {{ color: var(--theme-color); font-size: 2.8rem; font-weight: 300; line-height: 1; letter-spacing: -1px; }}
    .kpi-value span {{ font-size: 1.2rem; color: var(--text-muted); margin-left: 4px; font-weight: 400; }}
    ...
    /* Expander as Glass Card (Gallery) */
    ...
    [data-testid="stExpander"] summary {{
        ...
        font-weight: 500;
        color: var(--text-main);
    }}[data-testid="stExpander"] summary:hover {{ color: var(--theme-color); }}[data-testid="stExpander"] [data-testid="stMarkdownContainer"] p {{ font-size: 1.1rem; font-weight: 600; }}

    /* Custom Progress Bars */
    .pg-track {{ width: 100%; height: 6px; background: rgba(0,0,0,0.05); border-radius: 3px; overflow: hidden; margin: 12px 0; }}
    .pg-fill {{ height: 100%; border-radius: 3px; transition: width 1s ease; }}
    .pg-label {{ display: flex; justify-content: space-between; font-size: 0.85rem; color: var(--text-muted); font-weight: 500; }}
    .pg-title {{ font-size: 1.2rem; font-weight: 600; color: var(--text-main); margin-bottom: 4px; }}

    /* Typography */
    .section-title {{ font-size: 1.2rem; font-weight: 500; color: var(--text-main); margin-bottom: 24px; letter-spacing: -0.5px; }}
    
    /* Radio Buttons (Time Filter) */
...
# ==========================================
# 4. Sidebar: Laboratory
# ==========================================
with st.sidebar:
    st.markdown("<h2 style='font-weight: 500; margin-bottom: 32px; letter-spacing: -0.5px;'>Laboratory</h2>", unsafe_allow_html=True)
    ...
    with st.expander("Create", expanded=False):
        st.markdown("<div style='font-size: 0.85rem; color: #86868B; margin-bottom: 8px;'>Parent Subject</div>", unsafe_allow_html=True)
        ...
        st.markdown("<hr style='margin: 16px 0; border: none; border-top: 1px solid rgba(0,0,0,0.05);'>", unsafe_allow_html=True)
        
        st.markdown("<div style='font-size: 0.85rem; color: #86868B; margin-bottom: 8px;'>Child Task</div>", unsafe_allow_html=True)
        if config["subjects"]:
...
# ==========================================
# 6. Central Core L1: Absolute Balance
# ==========================================
...
with col_l1_left:
    ...
    if not parent_subjects:
        st.markdown("<div style='color: var(--text-muted); padding-top: 10px;'>Awaiting Configuration</div>", unsafe_allow_html=True)
...
with col_l1_center:
    if parent_subjects:
        if st.session_state.timer_state == 'idle':
            st.markdown(f"""
                <div style="text-align: center; font-family: 'SF Pro Display', 'Helvetica Neue', monospace; font-size: 4.5rem; font-weight: 200; color: var(--text-main); opacity: 0.8; line-height: 1; margin-bottom: 16px; letter-spacing: -2px;">
                    00:00:00
                </div>
            """, unsafe_allow_html=True)
            ...
            timer_html = f"""
            <style>
                body {{ margin: 0; display: flex; justify-content: center; background: transparent; color: {theme_color}; font-family: 'SF Pro Display', 'Helvetica Neue', monospace; font-size: 4.5rem; font-weight: 200; line-height: 1; letter-spacing: -2px; }}
            </style>
...
with col_l3_left:
    ...
    for idx, (parent, details) in enumerate(config["subjects"].items()):
        ...
        with gallery_cols[idx % 2]:
            # The expander acts as the card itself
            with st.expander(f"{parent} — {current_h:.1f}h / {target_h}h"):
                st.markdown(f"""
                <div class="pg-title">{parent}</div>
                <div class="pg-track">
                    <div class="pg-fill" style="width: {progress_pct}%; background-color: {card_color};"></div>
                </div>
                    ...
                    for child, c_details in children.items():
                        ...
                        <div style="margin-top: 12px;">
                            ...
                            <div class="pg-track" style="height: 4px; margin: 6px 0;">
                                <div class="pg-fill" style="width: {c_pct}%; background-color: {card_color}; opacity: 0.7;"></div>
                            </div>
                        </div>
                        """
...
with col_l3_right:
    ...
    fig_gauge = go.Figure(go.Indicator(
        ...
        gauge = {
            'axis': {'range':[None, max(t_hours, y_hours, 8)], 'tickwidth': 1, 'tickcolor': "rgba(255,255,255,0)"},
    ...
    fig_gauge.update_layout(height=180, margin=dict(l=20, r=20, t=30, b=10), paper_bgcolor='rgba(0,0,0,0)', font={'family': "-apple-system, sans-serif"})
    ...
    if not filtered_df.empty:
        ...
        fig_pie.update_traces(textposition='inside', textinfo='percent', marker=dict(line=dict(color='rgba(255,255,255,0.5)', width=1)))
        fig_pie.update_layout(
            showlegend=True, legend=dict(orientation="h", yanchor="bottom", y=-0.2, xanchor="center", x=0.5),
            height=220, margin=dict(l=10, r=10, t=10, b=10), paper_bgcolor='rgba(0,0,0,0)', font={'family': "-apple-system, sans-serif"}
        )
        ...
        st.markdown("<div style='height: 220px; display: flex; align-items: center; justify-content: center; color: var(--text-muted);'>No data available</div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
fig_heat = go.Figure(data=go.Heatmap(
    ...
    x=heatmap_df['x'],
    y=heatmap_df['y'],
    colorscale=[[0, 'rgba(0,0,0,0.04)'], [1, theme_color]],
    xgap=4, ygap=4,
    showscale=False,
    hoverinfo='text',
