...
bg_gradient = f"linear-gradient(135deg, {theme_color}33 0%, #d1d9e6 50%, #8e9eab 100%)"
...
<style>
    ...
    /* Global Deep Liquid Glass Background */
    .stApp {{
    ...
    .kpi-value {{ color: var(--theme-color); font-family: 'Outfit', sans-serif; font-size: 3.5rem; font-weight: 700; line-height: 1; letter-spacing: -1px; text-shadow: 0 2px 10px rgba(255,255,255,0.3); }}
    ...
    .pg-track {{ width: 100%; height: 6px; background: rgba(255,255,255,0.4); border-radius: 3px; overflow: hidden; margin: 10px 0; box-shadow: inset 0 1px 2px rgba(0,0,0,0.05); }}
    ...
    /* Sidebar Glass */[data-testid="stSidebar"] {{
...
with st.sidebar:
    ...
    with st.expander("Create", expanded=False):
        ...
        st.markdown("<hr style='margin: 16px 0; border: none; border-top: 1px solid rgba(255,255,255,0.3);'>", unsafe_allow_html=True)
...
with col_l3_right:
    ...
    fig_gauge = go.Figure(go.Indicator(
        ...
        value = t_hours,
        title = {'text': "Today vs Yesterday (h)", 'font': {'size': 14, 'color': '#4A4A4A', 'family': 'Inter'}},
    ...
    if not filtered_df.empty and total_hours > 0:
        ...
        fig_pie.update_layout(
            title={'text': "Time Distribution", 'font': {'size': 14, 'color': '#4A4A4A', 'family': 'Inter'}, 'x': 0.5, 'xanchor': 'center'},
...
fig_heat.update_layout(
    ...
    yaxis=dict(
        ...
        autorange='reversed',
        tickfont=dict(color="#4A4A4A", family="Inter, sans-serif", size=12)
    )
)
