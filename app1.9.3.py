...
def sanitize_hex(color_str):
    """
    The Color Shield: Ensures Plotly receives a strictly 6-digit HEX code.
    Strips alpha channels (#RRGGBBAA -> #RRGGBB) to prevent ValueErrors.
    """
...
safe_theme_color = sanitize_hex(raw_theme_color) # Use this for Plotly

# Generate Palette for Pie Charts
...
palette = generate_palette(safe_theme_color, max(len(config["subjects"]), 5))

# Deep Liquid Gradient Logic
...
<style>
    ...
    /* 2. THE WHITE BAR EXTERMINATOR: Aggressive Transparency */
    /* Forces Streamlit containers, metrics, blocks, and headers to be transparent */
...
with col_l1_center:
    if parent_subjects:
        if st.session_state.timer_state == 'idle':
            ...
            # JS Timer for smooth counting without reruns
            timer_html = f"""
            <style>
...
# ==========================================
# 7.5 Weekly Report (Dialog)
# ==========================================
@st.dialog("Weekly Insight Report")
def show_report_dialog():
    # Calculate Weekly Stats
    start_week = now - timedelta(days=now.weekday())
    w_df = df[df['timestamp'].dt.date >= start_week.date()]
    ...
    <div style="text-align: center; padding: 20px;">
        <h3 style="color: {safe_theme_color}; font-family: 'Outfit'; margin-bottom: 24px;">Weekly Summary</h3>
        ...
        <p style="color: #666; font-style: italic;">"Consistency is the architecture of mastery."</p>
    </div>
    """, unsafe_allow_html=True)
...
with col_l3_right:
    ...
    st.markdown("<div class='glass-card' style='padding: 24px;'>", unsafe_allow_html=True)
    # Robust Plotly Fix: Empty data handling
