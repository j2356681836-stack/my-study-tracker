...
def sanitize_hex(color_str):
    """Ensure strictly 6-digit HEX for Plotly compatibility."""
...
# Monochromatic Palette Generator (Harmonious)
def generate_monochromatic_palette(base_hex, n=5):
    ...
    palette = []
    for i in range(n):
        # Vary lightness to create depth while keeping hue/saturation similar
        new_l = max(0.2, min(0.9, l + (i * 0.1) - (n * 0.05))) 
...
<style>
    ...
    .kpi-value {{ color: var(--theme-color); font-family: 'Outfit', sans-serif; font-size: 3.5rem; font-weight: 700; line-height: 1; letter-spacing: -1px; text-shadow: 0 4px 20px rgba(255,255,255,0.5); }}
...
# ==========================================
# 4. Sidebar: Management & Reporting
# ==========================================
df = pd.read_csv(DATA_FILE)
df['timestamp'] = pd.to_datetime(df['timestamp'])
now = datetime.now()

# --- Reporting System (MoM/YoY Logic) ---
@st.dialog("Achievement Report")
def show_report_dialog(period_type):
    # Determine date ranges for Current vs Previous
    ...
    # Growth Calculation
    if p_hours > 0:
        growth = ((c_hours - p_hours) / p_hours) * 100
    ...
    <div style="text-align: center; padding: 10px;">
        <h3 style="color: {safe_theme_color}; font-family: 'Outfit'; margin-bottom: 30px; letter-spacing: -0.5px;">{period_name} Report</h3>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 30px;">
            <div style="background: rgba(255,255,255,0.5); padding: 20px; border-radius: 20px;">
                ...
                <div style="font-size:2rem; font-weight:700; color:{safe_theme_color};">{c_hours:.1f}h</div>
                <div style="font-size:0.9rem; color:{growth_color}; font-weight:600;">{growth_str} vs prev</div>
            </div>
            <div style="background: rgba(255,255,255,0.5); padding: 20px; border-radius: 20px;">
                <div style="font-size:0.8rem; color:#888; text-transform:uppercase;">Top Subject</div>
                <div style="font-size:1.4rem; font-weight:600; color:#333; margin-top:5px; line-height:1.2;">{top_subj}</div>
            </div>
        </div>
        
        <div style="margin-bottom: 30px;">
             <div style="font-size:0.8rem; color:#888; text-transform:uppercase; margin-bottom:5px;">Average Focus</div>
             <div style="font-size:2.5rem; font-weight:700; color:#333;">{avg_focus:.1f}<span style="font-size:1.2rem; color:#888;"> / 5.0</span></div>
        </div>

        <p style="color: #666; font-style: italic; font-family: 'Inter';">"Design is not just what it looks like and feels like. Design is how it works."</p>
    </div>
    """, unsafe_allow_html=True)

with st.sidebar:
    st.markdown("<h2 style='font-family: Outfit; font-weight: 600; margin-bottom: 24px;'>Laboratory</h2>", unsafe_allow_html=True)
    
    # 1. Reporting Module
    with st.container():
        st.markdown("<div style='font-size: 0.85rem; color: var(--text-muted); font-weight: 600; margin-bottom: 8px;'>Intelligence</div>", unsafe_allow_html=True)
    ...
    with st.expander("Delete", expanded=False):
        if config["subjects"]:
            del_type = st.radio("Type", ["Parent", "Child"], horizontal=True, label_visibility="collapsed", key="del_radio")
            if del_type == "Parent":
...
# Apply Global Time Filter Logic
if time_filter == "Today":
    filtered_df = df[df['timestamp'].dt.date == now.date()]
    compare_start = now - timedelta(days=1) # Yesterday
    ...
    # Simplified Last Month logic
    last_month_date = now.replace(day=1) - timedelta(days=1)
    compare_df = df[(df['timestamp'].dt.year == last_month_date.year) & (df['timestamp'].dt.month == last_month_date.month)]
...
with col_l3_right:
    ...
    # Dynamic Gauge Logic based on Time Filter
    current_val = total_hours
    compare_val = compare_df['duration_minutes'].sum() / 60 if not compare_df.empty else 0
    
    # Robust Plotly Fix: range must strictly > 0.
    max_val = max(current_val, compare_val)
    gauge_max = max_val if max_val > 0 else 1.0

    fig_gauge = go.Figure(go.Indicator(
        mode = "gauge+number+delta",
        value = current_val,
        number = {'valueformat': ".2f"}, # Precision requirement
        title = {'text': f"{time_filter} vs {compare_label} (h)", 'font': {'size': 14, 'color': '#5A5A5E', 'family': 'Inter'}},
        delta = {'reference': compare_val, 'increasing': {'color': safe_theme_color}},
        gauge = {
            'axis': {'range': [0, gauge_max], 'tickwidth': 1, 'tickcolor': "rgba(255,255,255,0)"},
            'bar': {'color': safe_theme_color},
            'bgcolor': "rgba(255,255,255,0.3)",
            'borderwidth': 0,
            'threshold': {'line': {'color': f"{safe_theme_color}", 'width': 2}, 'thickness': 0.75, 'value': compare_val}
        }
    ))
